![malert](docs/2026-03-06_16-46-17.png "Всплывающая подсказка")
...
- [Сервис](#сервис)
  ...
  - [Режимы работы (single vs multi)](#режимы-работы-single-vs-multi)
      - [single-instance](#single-instance)
      - [multi-instance](#multi-instance)
    - [Входящие интерфейсы](#входящие-интерфейсы)
    - [Исходящие интерфейсы](#исходящие-интерфейсы)
  - [Целевая рабочая нагрузка и доступность](#целевая-рабочая-нагрузка-и-доступность)
  - [Фактическая  нагрузка](#фактическая--нагрузка)
    - [Стенд](#стенд)
    - [HTTP](#http)
    - [NATS](#nats)
- [Принцип работы](#принцип-работы)
  - [Входящие события](#входящие-события)
  - [Алертинг](#алертинг)
  - [Ключ алерта](#ключ-алерта)
  - [Фильтрация событий](#фильтрация-событий)
  - [Состояния алерта](#состояния-алерта)
    - [Out-of-order события](#out-of-order-события)
  - [Тип алерта "CountTotal"](#тип-алерта-counttotal)
  - [Тип алерта "CountWindow"](#тип-алерта-countwindow)
  - [Тип алерта "MissingHeartbeat"](#тип-алерта-missingheartbeat)
  - [Паттерны написания правил алертов](#паттерны-написания-правил-алертов)
    - [Общий каркас правила](#общий-каркас-правила)
- [Доставка уведомлений](#доставка-уведомлений)
...
- [Подготовка контура](#подготовка-контура)
  - [NATS](#nats-1)
  - [Запуск сервиса алертинга](#запуск-сервиса-алертинга)
...
После этапов фильтрации и агрегации входящих событий согласно типу алертов, сервис принимает решение 
- поднять алерт
...
Преимущества решения
  1. на порядки снижается трафик в алертинг за счет работы c агрегатами метрик (ClickHouse + mat.view), а не сырыми событиями
  1. алертинг работает в режиме близком реальному времени вне зависимости от количества данных. агрегаты формируются и передаются в nats в момент вставки новых данных в хранилище

### Произвольный 
Предполагает формирование агрегатов или сырых событий на стороне самих сервисов и доставку их в алертинг через nats или HTTP
...
##  Фильтрация событий
К выделению алерта допускаются события, которые прошли фильтр. Фильтр возможен по типу события E.type ∈ rule.types (например ["event","agg"]), по tags, по именам переменных,  и значениям переменных.
При этом:
- tags и имена переменных фильтруется только allow-фильтр по ключам и значениям. Если значение не попадает в allow, событие игнорируется
- значения переменных фильтруются по типу переменной:
value.t="n|s|b"
  t - тип переменной:
  - n: float64
  - s: string
...
Периоды без отправки уведомлений задаются на уровне правила через `[[rule.<rule_name>.notify.off]]`:
- в off-окне алерт продолжает формироваться, состояние/карточка/логи работают как обычно;
- подавляется только отправка уведомлений (direct и queue режимы);
- после выхода из окна догоняющая отправка не выполняется.
...
[[rule.<rule_name>.notify.off]]
# Опционально. в off-окне алерт считается как обычно, но уведомления не отправляются
# Дни недели (mon..sun), по локальной timezone хоста.
days = ["mon", "tue", "wed", "thu", "fri"]
# Начало окна без отправки уведомлений.
from = "22:00"
# Конец окна без отправки уведомлений (допускается "24:00").
to = "08:00"
...
Встроенная морда настраивается в `configs/ui.toml`.

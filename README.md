...
- Режим источника конфигов выбирается один:
  - либо один файл с набором правил;
  - либо директория с несколькими rule-файлами.
- Hot reload обязателен.
- На reload делается предварительная полная валидация нового снапшота:
  - парсинг всех правил;
  - проверка уникальности `rule_name` (дубликаты -> ошибка);
  - структурная и семантическая проверка.
- Применение нового снапшота только атомарно: если валидация не прошла, остается предыдущая рабочая конфигурация.
- Состояния алертов хранятся в NATS KV; сам reload не изменяет их автоматически.
- Если `rule_name` не меняется, текущее состояние сохраняется, а новые параметры применяются только к новым событиям (без ретроспективного пересчета).
- Опциональные guard-параметры runtime-state (по умолчанию отключены):
  - `service.runtime_state_idle_sec` (`0` = off) — удалять неактивные runtime-state по idle TTL.
  - `service.runtime_state_max` (`0` = off) — ограничивать максимальное число runtime-state, удаляя самые старые неактивные.

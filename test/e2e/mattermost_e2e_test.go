...
`, ruleName))
			...
			if !strings.Contains(first.Message, ruleName+" firing") {
			...
			if !strings.Contains(second.Message, ruleName+" resolved") {
...
func TestMattermostWebhookRetryOnTransientFailureE2E(t *testing.T) {
	for _, metric := range e2eFunctionalMetricCases() {
		...
		t.Run(metric.Name, func(t *testing.T) {
			...
			ruleName := "mm_retry_" + metric.Name
			ruleOptions := defaultE2ERuleOptions(metric)
			switch metric.AlertType {
			case "count_total", "count_window":
				ruleOptions.ResolveSilenceSec = 3600
			default:
				ruleOptions.MissingSec = 1
			}
